...
if __name__ == '__main__':
    ...
    if args.shorts:
        ...
        stocks.append_to_table(data_dir, data=short_interest, date_str=today, name="short interest")
    ...
    if args.changes:
        ...
        stocks.append_to_table(data_dir, data=max_change, date_str=today, name="max intraday change")

        days_since_max = stocks.days_since_max_spike(intraday_change, tickers)
        stocks.append_to_table(data_dir, data=days_since_max, date_str=today, name="days since max spike")
    ...
    if args.indicators:
        ...
        stocks.append_to_table(data_dir, data=formatted_rsi, date_str=today, name="RSI")
        ...
        stocks.append_to_table(data_dir, data=formatted_bb, date_str=today, name="BB value")

        MACD = stocks.get_MACD(prices)
        formatted_MACD = stocks.format_data(MACD, tickers, name="macd")
        stocks.append_to_table(data_dir, data=formatted_MACD, date_str=today, name="MACD")
        ...
        stocks.append_to_table(data_dir, data=formatted_ichi, date_str=today, name="Ichimoku cloudd")

    if args.news:
        # News sentiment
        news_sentiment = stocks.scrape_news_sentiment(tickers)
        stocks.append_to_table(data_dir, data=news_sentiment, date_str=today, name="news sentiment")
    ...
    if args.options:
        ...
        stocks.append_to_table(data_dir, data, today, name="call-put ratio")

        pool = mp.Pool(num_proc)
        with pool:
            data = pool.map(stocks.get_put_call_magnitude, tickers_split)
        data = pd.concat(data, axis=0)
        stocks.append_to_table(data_dir, data, today, name="call-put value ratio")

    if args.earnings:
        dte = stocks.gather_DTE(tickers)
        stocks.append_to_table(data_dir, dte, today, name="days to earnings")